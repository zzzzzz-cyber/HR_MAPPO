...
class CCMADDPG(object):
    def __init__(self, InfdexofResult, env, env_eval, n_agents, state_dim, action_dim, action_lower_bound, action_higher_bound,
                 memory_capacity=10000, target_tau=0.05, reward_gamma=0.9, reward_scale=0.1, done_penalty=None,
                 ...
                 epsilon_start=1, epsilon_end=0.01, epsilon_decay= None, use_cuda=False, Benchmarks_mode="MADDPG"):
    ...
    def interact(self, MAX_EPISODES, EPISODES_BEFORE_TRAIN, NUMBER_OF_EVAL_EPISODES):
        while self.n_episodes < MAX_EPISODES:
            ...
            if self.n_episodes % EPISODES_BEFORE_TRAIN == 0:
                self.train() 
                pass
    ...
    def train(self):
        ...
        for agent_id_actor in range(self.n_agents):
            newactor_actions = [] 
            # Calculate next target actions for each agent
            for agent_id_next in range(self.n_agents):
                newactor_action_var = self.actors[agent_id_next](states_var[:, agent_id_next, :])
                if self.use_cuda:
