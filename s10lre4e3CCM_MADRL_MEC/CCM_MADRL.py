...
class CCM_MADDPG(object):
    def __init__(self, InfdexofResult, env, env_eval, n_agents, state_dim, action_dim, action_lower_bound, action_higher_bound,
                 memory_capacity=10000, target_tau=0.5, reward_gamma=0.9, reward_scale=1., done_penalty=None,
                 actor_output_activation=torch.tanh, actor_lr=1e-4, critic_lr=1e-3,
                 optimizer_type="adam", max_grad_norm=0.2, batch_size=64, episodes_before_train=64,
    ...
    def interact(self, MAX_EPISODES, EPISODES_BEFORE_TRAIN, NUMBER_OF_EVAL_EPISODES):
        # self.loss_min_max = self.load_min_max()
        while self.n_episodes < MAX_EPISODES:
            ...
            while not done:  # 每10次作为一个episodes
                ...
                # global_reward = (global_reward - global_reward.mean()) / (global_reward.std() + 1e-10)
                # =====================global=====================
    ...
    def append_tree_sample(self, states, actor_actions, critic_actions, rewards, next_states, dones):
        ...
        for agent_id in range(self.n_agents):
            next_action_var = self.actors[agent_id](next_states_var[:, agent_id, :])
            if self.use_cuda:
                nextactor_actions.append(next_action_var.data.cuda())
            else:
        ...
        for nexta in range(self.n_agents):  # to find the maxQ
            if nextactor_actions_var[0, nexta, 0] >= 0:
                nextperQs.append(self.critics_target[0](whole_next_states_var[0], whole_nextactor_actions_var[0], next_states_var[0, nexta, :], nextactor_actions_var[0, nexta, :]).detach())
        if len(nextperQs) == 0:
        ...
        for a in range(self.n_agents):
            if critic_actions_var[0, a, 0] == 1:
                # current prediction
                curr_perQ = self.critics[0](whole_states_var[0], whole_actor_actions_var[0], states_var[0, a, :], actor_actions_var[0, a, :]).detach()
                error += (tar - curr_perQ)**2
                cselected += 1
        if cselected != 0:
            error = error / cselected
        if cselected == 0:  # if all tasks were allocated locally, the feedback should be sent using the commbined local decision and a fake perAgent that learns the best Q value for that situation
            curr_perQ = self.critics[0](whole_states_var[0], whole_actor_actions_var[0], torch.zeros(self.state_dim), torch.zeros(self.action_dim)).detach()
            error += (tar - curr_perQ)**2
    ...
    def train(self):
        ...
        if self.use_tree:
            ...
            # errors = np.zeros(self.batch_size)
            # is_weights = to_tensor_var(np.ones((self.batch_size, 1)), self.use_cuda)
        ...
        whole_states_var = states_var.view(-1, self.n_agents*self.state_dim)
        whole_actor_actions_var = actor_actions_var.view(-1, self.n_agents*self.action_dim)
        whole_next_states_var = next_states_var.view(-1, self.n_agents*self.state_dim)  # 3
        ...
        whole_next_actor_actions_var = next_actor_actions_var.view(-1, self.n_agents*self.action_dim)   # 4
        #common critic
        agent_id = 0
        target_q = []
        current_q = []
        for b in range(self.batch_size):
            ...
            nextperQs = []
            for nexta in range(self.n_agents):#to find the maxQ
                if next_actor_actions_var[b, nexta, 0] >= 0:
                    nextperQs.append(self.critics_target[agent_id](whole_next_states_var[b], whole_next_actor_actions_var[b], next_states_var[b, nexta, :], next_actor_actions_var[b, nexta, :]))   # 7

            if len(nextperQs) == 0:
                tar_perQ = self.critics_target[agent_id](whole_next_states_var[b], whole_next_actor_actions_var[b], torch.zeros(self.state_dim), torch.zeros(self.action_dim))   # 11
            else:
                tar_perQ = max(nextperQs)   # 13
            ...
            for a in range(self.n_agents):
                if critic_actions_var[b, a, 0] == 1:
                    #current prediction
                    curr_perQ = self.critics[agent_id](whole_states_var[b], whole_actor_actions_var[b], states_var[b, a, :], actor_actions_var[b, a, :])
                    current_q.append(curr_perQ*is_weights[b])
                    target_q.append(tar*is_weights[b])
                    errors[b] += (curr_perQ-tar)**2
                    cselected += 1
            if cselected != 0:
                errors[b] = errors[b] / cselected
            if cselected == 0:# if all tasks were allocated locally, the feedback should be sent using the commbined local decision and a fake perAgent that learns the best Q value for that situation
                curr_perQ = self.critics[agent_id](whole_states_var[b], whole_actor_actions_var[b], torch.zeros(self.state_dim), torch.zeros(self.action_dim))
                current_q.append(curr_perQ*is_weights[b])
                target_q.append(tar*is_weights[b])
                errors[b] += (curr_perQ-tar)**2
        ...
        # self.critics_target[0].load_state_dict(self.critics[0].state_dict())
        ...
        whole_new_actor_actions_var = new_actor_actions_var.view(-1, self.n_agents*self.action_dim)  # 32
        actor_loss = []
        ...
        for b in range(self.batch_size):
            ...
            for a in range(self.n_agents):
                if critic_actions_var[b, a, 0] == 1:  # if it was selected
                    perQ = self.critics[0](whole_states_var[b], whole_new_actor_actions_var[b], states_var[b, a, :], new_actor_actions_var[b, a, :])
                    Qselected.append(perQ*is_weights[b])
            if len(Qselected) == 0:#if all tasks were allocated locally, the feedback should be sent using the commbined local decision and a fake perAgent that learns the best Q value for that situation
                perQ = self.critics[0](whole_states_var[b], whole_new_actor_actions_var[b], torch.zeros(self.state_dim), torch.zeros(self.action_dim))
        ...
        actor_loss.backward()
        if self.max_grad_norm is not None:
            torch.nn.utils.clip_grad_norm_(self.actors[0].parameters(), self.max_grad_norm)
        self.actors_optimizer[0].step()
        self._soft_update_target(self.actors_target[0], self.actors[0])
        # self.actors_target[0].load_state_dict(self.actors[0].state_dict())

        for agent_id in range(1, self.n_agents):
            self.actors[agent_id].load_state_dict(self.actors[0].state_dict())
            self.actors_target[agent_id].load_state_dict(self.actors_target[0].state_dict())
    ...
    def lr_decay(self, total_steps):  # Trick 6: learning rate Decay
        ...
        for p, c in zip(self.actors_optimizer[0].param_groups, self.critics_optimizer[0].param_groups):
            p['lr'] = lr_now
            c['lr'] = clr_now
        '''
    ...
    def save_models(self, path):
        ...
        torch.save(self.critics[0].state_dict(), path + str(self.InfdexofResult) + '-critics.pth')

    def load_models(self, path):
        self.actors[0].load_state_dict(torch.load(path + str(self.InfdexofResult) + "-actors.pth"))
        self.critics[0].load_state_dict(torch.load(path + str(self.InfdexofResult) + "-critics.pth"))
    ...
    def choose_action(self, state, evaluation):
        ...
        o_state = state.copy()
        state_var = self.env.preprocessing(o_state)
        ...
        # for agent_id in range(self.n_agents):
        action_var = self.actors[0](state_var)
        if self.use_cuda:
    ...
    def evaluate(self, EVAL_EPISODES):
        ...
        for i in range(EVAL_EPISODES):
            ...
            if i == EVAL_EPISODES-1 and done:
                ...
                savetxt('./CSV/result_EC/CCM_MADRL'+str(self.InfdexofResult)+'.csv', result_EC)
                savetxt('./CSV/result_EX/CCM_MADRL'+str(self.InfdexofResult)+'.csv', result_EX)
                savetxt('./CSV/result_LT/CCM_MADRL'+str(self.InfdexofResult)+'.csv', result_LT)
                savetxt('./CSV/result_LX/CCM_MADRL' + str(self.InfdexofResult)+'.csv', result_LX)
