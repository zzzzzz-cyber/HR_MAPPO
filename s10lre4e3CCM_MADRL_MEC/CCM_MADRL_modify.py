...
class CCM_MADDPG(object):
    ...
    def train(self):
        ...
        if self.use_tree:
            tryfetch = 0
            while tryfetch < 3:
                mini_batch, idxs, is_weights = self.memory.tree_sample(self.batch_size)
        ...
        for b in range(self.batch_size):
            ...
            for nexta in range(self.n_agents):#to find the maxQ
                if next_actor_actions_var[b, nexta, 0] >= 0:
                    nextperQs.append(self.critics_target[agent_id](whole_next_states_var[b], whole_next_actor_actions_var[b], next_states_var[b, nexta, :], next_actor_actions_var[b, nexta, :]))

            if len(nextperQs) == 0:
                tar_perQ = self.critics_target[agent_id](whole_next_states_var[b], whole_next_actor_actions_var[b], torch.zeros(self.state_dim), torch.zeros(self.action_dim))
            else:
                tar_perQ = max(nextperQs)
        ...
        for agent_id_actor in range(self.n_agents):
            ...
            for b in range(self.batch_size):
                ...
                if critic_actions_var[b, agent_id_actor, 0] == 1:  # if it was selected
                    perQ = self.critics[0](whole_states_var[b], whole_new_actor_actions_var[b], states_var[b, agent_id_actor, :], new_actor_actions_var[b, agent_id_actor, :])
                    Qselected.append(perQ)
