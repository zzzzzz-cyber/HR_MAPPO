...
class COM_DDPG(object):
    ...
    def train(self):
        ...
        dones_var = dones_var.repeat(1, 1, self.n_agents).view(-1, self.n_agents, 1)
        ...
        whole_next_action = next_action_var.reshape(self.batch_size, -1)

        next_continue_Q_value = self.target_critic[0](whole_next_states_var, whole_next_action)
        target_continue_Q_value = self.reward_scale * rewards_var[:, 0, :] + self.reward_gamma * next_continue_Q_value
        current_continue_Q_value = self.critic[0](whole_states_var, whole_actor_actions_var)
        DDPG_critic_loss = torch.mean(MSE(target_continue_Q_value, current_continue_Q_value))
        ...
        whole_action = new_action_var.reshape(self.batch_size, -1)

        actor_Q = self.critic[0](whole_states_var, whole_action)
    ...
    def evaluate(self, EVAL_EPISODES):
        ...
        for i in range(EVAL_EPISODES):
            ...
            if i == EVAL_EPISODES-1 and done:
                ...
                savetxt('./CSV/results/COM_DDPG_'+str(self.InfdexofResult)+'.csv', arrayresults)
                savetxt('./CSV/loss/COM_DDPG_'+str(self.InfdexofResult)+'.csv', arrayloss)
                savetxt('./CSV/Server_constraints/COM_DDPG_'+str(self.InfdexofResult)+'.csv', arrayserver)
                savetxt('./CSV/Energy_constraints/COM_DDPG_'+str(self.InfdexofResult)+'.csv', arrayenergy)
                savetxt('./CSV/Time_constraints/COM_DDPG_' + str(self.InfdexofResult)+'.csv', arraytime)
    ...
    def evaluateAtTraining(self, EVAL_EPISODES):
        ...
        savetxt('./CSV/AtTraining/COM_DDPG_' + self.InfdexofResult+'.csv', arrayresults)
