...
from Model_mu_std import ActorNetwork, decentralized_CriticNetwork
...
class IPPO(object):
    def __init__(self, InfdexofResult, env, env_eval, n_agents, state_dim, action_dim, action_lower_bound, action_higher_bound,
                 ...
                 actor_output_activation=torch.tanh, actor_lr=1e-5, critic_lr=1e-4,
        ...
        self.critics = [decentralized_CriticNetwork(self.state_dim)] * self.n_agents
        ...
        while self.n_episodes < MAX_EPISODES:
            ...
            while not done:  # 每10次作为一个episodes
                ...
                self.replay_buffer.store_transition(episode_step, min_max_state, global_min_max_state,
                                                    global_reward, local_reward, original_reward, actor_action,
                                                    min_max_next_state, global_min_max_next_state, done)
            ...
            if self.replay_buffer.episode_num == self.episodes_before_train:  # 在交换环境一定次数后开始训练
                ...
                tmp = self.replay_buffer.buffer['loc_r_n']
                self.replay_buffer.buffer['loc_r_n'] = ((tmp - tmp.mean()) / (tmp.std() + 1e-10))
                self.train()
    ...
    def train(self):
        ...
        for agent_id in range(self.n_agents):
            value = self.critics[agent_id](batch['loc_obs_c_agent_n'][:, :, agent_id, :])  # 64, 10, agent_id
            deltas = self.reward_scale * batch['loc_r_n'][:, :, agent_id, :] + \
                     self.reward_gamma * self.critics[agent_id](batch['loc_obs_n_agent_n'][:, :, agent_id, :]) - value  # 64, 10, agent_id
            ...
            advantage_actor = ((advantage - advantage.mean()) / (advantage.std() + 1e-5)).detach()

            old_action_mu, old_action_std = self.actors[agent_id](batch['loc_obs_c_agent_n'][:, :, agent_id, :])
            old_policy_dist = torch.distributions.Normal(old_action_mu.detach(), old_action_std.detach())
            old_log_prob = old_policy_dist.log_prob(batch['action_agent_n'][:, :, agent_id, :])
            comb_old_log_prob = old_log_prob.sum(dim=-1, keepdims=True)

            for _ in range(self.epochs):
                # critic updates
                new_value = self.critics[agent_id](batch['loc_obs_c_agent_n'][:, :, agent_id, :])
                ...
                # actor update
                # 每一轮更新一次策略网络预测的状态
                new_action_mu, new_action_std = self.new_actors[agent_id](batch['loc_obs_c_agent_n'][:, :, agent_id, :])
                new_policy_dist = torch.distributions.Normal(new_action_mu, new_action_std)
                new_log_prob = new_policy_dist.log_prob(batch['action_agent_n'][:, :, agent_id, :])
                comb_new_log_prob = new_log_prob.sum(dim=-1, keepdims=True)
                # 新旧策略之间的比例
                ratio = (comb_new_log_prob.exp() + 1e-10) / (comb_old_log_prob.exp() + 1e-10)
                # 近端策略优化裁剪目标函数公式的左侧项
            ...
            if proposed > constraint or sumofproposed > S_E:  # if the number of agents proposed to offload is greater than the number of available channels
                ...
                for agentid in range(self.n_agents):
                    if actor_action[agentid, 0] >= 0:
                        ...
                        action_Qs[agentid] = actor_action[agentid, 0]
    ...
    def evaluate(self, EVAL_EPISODES):
        ...
        for i in range(EVAL_EPISODES):
            ...
            if i == EVAL_EPISODES-1 and done:
                ...
                savetxt('./CSV/results/IPPO_'+str(self.InfdexofResult)+'.csv', arrayresults)
                savetxt('./CSV/loss/IPPO_'+str(self.InfdexofResult)+'.csv', arrayloss)
                savetxt('./CSV/Server_constraints/IPPO_'+str(self.InfdexofResult)+'.csv', arrayserver)
                savetxt('./CSV/Energy_constraints/IPPO_'+str(self.InfdexofResult)+'.csv', arrayenergy)
                savetxt('./CSV/Time_constraints/IPPO_' + str(self.InfdexofResult)+'.csv', arraytime)
    ...
    def evaluateAtTraining(self, EVAL_EPISODES):
        ...
        savetxt('./CSV/AtTraining/IPPO_' + self.InfdexofResult+'.csv', arrayresults)
