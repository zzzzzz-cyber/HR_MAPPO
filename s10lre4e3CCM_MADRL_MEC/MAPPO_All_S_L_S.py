    ...
    def interact(self, MAX_EPISODES, EPISODES_BEFORE_TRAIN, NUMBER_OF_EVAL_EPISODES):
        while self.n_episodes < MAX_EPISODES:
            ...
            if self.replay_buffer.episode_num == self.episodes_before_train:  # 在交换环境一定次数后开始训练
                ...
                tmp = self.replay_buffer.buffer['loc_reward_n']
                self.replay_buffer.buffer['loc_reward_n'] = (tmp - tmp.mean()) / (tmp.std() + 1e-10)
    ...
    def train(self):
        ...
        if self.use_critic_RNN:
            ...
            for t in range(MAX_STEPS):
                tmp_current_value = self.critics[0](batch['loc_current_obs_n'][:, t].reshape(self.batch_size * self.n_agents, -1))
                tmp_current_value = tmp_current_value.reshape(self.batch_size, self.n_agents, -1)
                value.append(tmp_current_value)
            value = torch.stack(value, dim=1)

            next_value = []
            self.critics[0].rnn_hidden = None
            for t in range(MAX_STEPS):
                tmp_next_value = self.critics[0](batch['loc_obs_next_n'][:, t].reshape(self.batch_size * self.n_agents, -1))
            ...
            next_value = self.critics[0](batch['loc_current_obs_n'])
            deltas = self.reward_scale * batch['loc_reward_n'] + self.reward_gamma * next_value - value
        ...
        for _ in range(self.epochs):
            ...
            for t2 in range(MAX_STEPS):
                new_action_mu_tmp, new_action_std_tmp = self.new_actors[0](batch['loc_current_obs_n'][:, t2].reshape(self.batch_size * self.n_agents, -1))
                new_action_mu = new_action_mu_tmp.reshape(self.batch_size, self.n_agents, -1)
                new_action_std = new_action_std_tmp.reshape(self.batch_size, self.n_agents, -1)
            ...
            comb_old_log_prob = batch['loc_current_log_act_n'].sum(dim=-1, keepdims=True)
