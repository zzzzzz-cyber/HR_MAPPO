...
class MAPPO_TFM(object):
    def __init__(self, InfdexofResult, env, env_eval, n_agents, state_dim, action_dim, action_lower_bound, action_higher_bound,
        ...
        #  ====================actor====================
        self.actors = [Encoder(self.discrete_action_dim, self.env_continue_action_dim, self.state_dim, 256, 32, 1, 8, dropout=0)] * 1
        #  ====================actor====================
    ...
    def Data_collection(self):
        ...
        while not done:  #  每8次作为一个episodes
    ...
    def train(self):
        ...
        discrete_action = to_tensor_var(batch['loc_current_discrete_n'], self.use_cuda, dtype="long")
        for _ in range(self.actors_epochs):
            new_action1 = self.new_actors[0].discrete_action(batch['loc_current_obs_n'])
            ...
            embedding_discrete = self.new_actors[0].embedding(discrete_action).reshape(self.batch_size, self.n_agents, -1)
            new_action2_mu, new_action2_std = self.new_actors[0].continue_action(embedding_discrete, batch['loc_current_obs_n'], mask=None)
    ...
    def choose_action(self, original_state, evaluation):
        ...
        action_var_prob = self.actors[0].discrete_action(state_var)[0]
        policy_dist1 = torch.distributions.Categorical(action_var_prob)
        action_var1 = policy_dist1.sample()

        discrete_action = action_var1.reshape(-1, 1)
        discrete_action_prob = torch.gather(action_var_prob, 1, discrete_action)
            ...
            if proposed > constraint or sumofproposed > S_E:  # if the number of agents proposed to offload is greater than the number of available channels
                ...
                for agentid in range(self.n_agents):
                    if actor_action[agentid, 0] > 0:
                        action_Qs[agentid] = discrete_action_prob[agentid]
        ...
        log_actor_action1 = to_tensor_var(log_actor_action1, self.use_cuda).reshape(-1, 1)

        e = torch.LongTensor(critic_action)
        current_discrete = to_tensor_var(critic_action, self.use_cuda).reshape(-1, 1)
        e_k = self.actors[0].embedding(e)
        e_k = e_k.unsqueeze(0)
        action_mu2, action_std2 = self.actors[0].continue_action(e_k, state_var, mask=None)
        ...
        current_continue = action_var2[0]
        current_continue = torch.clamp(current_continue, -1, 1)

        action_var = torch.cat([current_discrete, current_continue], -1)
        log_actor_action = torch.cat([log_actor_action1, log_action_tanh[0]], -1)

        current_discrete = critic_action.reshape(-1, 1)
        if self.use_cuda:
            actor_action = action_var.data.cpu().numpy()
            log_actor_action = log_actor_action.data.cpu().numpy()
        else:
            actor_action = action_var.data.numpy()
            log_actor_action = log_actor_action.data.numpy()
    ...
    def evaluate(self, EVAL_EPISODES):
        ...
        for i in range(EVAL_EPISODES):
            ...
            if i == EVAL_EPISODES-1 and done:
                ...
                savetxt('./CSV/results/MAPPO_TFM_'+str(self.InfdexofResult)+'.csv', arrayresults)
                savetxt('./CSV/loss/MAPPO_TFM_'+str(self.InfdexofResult)+'.csv', arrayloss)
                savetxt('./CSV/Server_constraints/MAPPO_TFM_'+str(self.InfdexofResult)+'.csv', arrayserver)
                savetxt('./CSV/Energy_constraints/MAPPO_TFM_'+str(self.InfdexofResult)+'.csv', arrayenergy)
                savetxt('./CSV/Time_constraints/MAPPO_TFM_' + str(self.InfdexofResult)+'.csv', arraytime)
    ...
    def evaluateAtTraining(self, EVAL_EPISODES):
        ...
        savetxt('./CSV/AtTraining/MAPPO_TFM_' + self.InfdexofResult+'.csv', arrayresults)
