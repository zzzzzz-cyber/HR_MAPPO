...
class MAPPO_VAE(object):
    def __init__(self, InfdexofResult, env, env_eval, n_agents, state_dim, action_dim, action_lower_bound, action_higher_bound,
        ...
        self.pca = None
        self.discrete_action_dim = 12
        self.continue_action_dim = 6
    ...
    def interact(self, MAX_EPISODES, EPISODES_BEFORE_TRAIN, NUMBER_OF_EVAL_EPISODES):
        if self.use_VAE3 or self.use_VAE2:
            buffer_address = "./CSV/min_max/MAPPO1_VAE_vae_buffer.pickle"
    ...
    def Data_collection(self):
        ...
        while not done:  # 每10次作为一个episodes
            ...
            # if self.use_VAE3 or self.use_VAE2:
            #     if self.replay_buffer.episode_num == 0:
    ...
    def vae_train(self):
        for _ in range(60):
            buffer_data = self.memory.rand_sample(32)
            state, actor_action, next_state = zip(*buffer_data)
    ...
    def vae_train2(self):
        ...
        # buffer_data = buffer_data[0:6400]
        if self.preheating:

            eplosde = 10
        else:
            # buffer_data = self.memory.rand_sample(6400)
            eplosde = 100
        for _ in range(eplosde):
            for index in BatchSampler(SequentialSampler(range(len(buffer_data))), 64, False):
                ...
                loss.backward()
                self.optimizer_vae.step()
        # z_buffer_data = torch.stack(z_buffer, dim=0)
        # z_buffer_data_np = z_buffer_data.detach().cpu().numpy()
        # z_buffer_data_np = z_buffer_data_np.reshape(-1, self.continue_action_dim)
        # pca = PCA(n_components=2)
        # pca.fit(z_buffer_data_np)
        # self.pca = pca

    def train(self):
        epsilon = self.epsilon_end + (self.epsilon_start - self.epsilon_end) * \
                  np.exp(-1. * self.n_episodes / self.epsilon_decay)
        noise = np.random.randn(self.batch_size, MAX_STEPS, self.n_agents, self.noise_dim) * epsilon
        ...
        for _ in range(self.critics_epochs):
            ...
            if self.use_critic_RNN:
                ...
                for t in range(MAX_STEPS):
                    tmp_new_value = self.new_critics[0](batch['loc_current_obs_n'][:, t].reshape(self.batch_size * self.n_agents, -1))
        ...
        if self.use_3_tanh or self.use_VAE:
        ...
        for _ in range(self.actors_epochs):
            if self.use_Actor_RNN_soft2:
                ...
                new_log_prob1 = new_policy_dist1.log_prob(batch['loc_current_discrete_n'].reshape(self.batch_size, MAX_STEPS, self.n_agents, -1))
                new_log_prob1 = new_log_prob1.sum(dim=-1, keepdims=True)

                new_policy_dist2 = torch.distributions.Normal(new_action2_mu, new_action2_std)
                new_log_prob2 = new_policy_dist2.log_prob(batch['loc_current_continue_n'].reshape(self.batch_size, MAX_STEPS, self.n_agents, -1))
                ...
                new_policy_dist1 = torch.distributions.Categorical(new_action1)
                new_log_prob1 = new_policy_dist1.log_prob(batch['loc_current_discrete_n'][:, :, :, 0]).reshape(self.batch_size, MAX_STEPS, self.n_agents, -1)
                ...
                new_policy_dist2 = torch.distributions.Normal(new_action2_mu, new_action2_std)
                new_log_prob2 = new_policy_dist2.log_prob(batch['loc_current_continue_n'][:, :, :, :self.continue_action].reshape(self.batch_size, MAX_STEPS, self.n_agents, -1))
                new_log_prob2 = new_log_prob2

                new_policy_dist3 = torch.distributions.Normal(new_action3_mu, new_action3_std)
                new_log_prob3 = new_policy_dist3.log_prob(batch['loc_current_continue_n'][:, :, :, self.continue_action:].reshape(self.batch_size, MAX_STEPS, self.n_agents, -1))
