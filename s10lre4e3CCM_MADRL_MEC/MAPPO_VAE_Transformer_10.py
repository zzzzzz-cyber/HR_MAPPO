...
PATH = "./model/HYASP_RMAPPO_10/"


class MAPPO_VAE_Trans_10(object):
    def __init__(self, InfdexofResult, env, env_eval, n_agents, state_dim, action_dim, action_lower_bound, action_higher_bound,
                 ...
                 epsilon_start=1, epsilon_end=0.01, epsilon_decay=None, use_cuda=True):
        ...
        self.pca = None
        self.discrete_action_dim = 9
        self.continue_action_dim = 18
        self.continue_action = 9
        ...
        #  ====================actor====================
        if self.use_VAE:
            self.actors = [ActorNetwork_ppovae3_M(self.state_dim, self.discrete_action_dim, self.continue_action, self.actor_output_activation)] * 1
            self.VAE = VAE(self.discrete_action_dim, self.continue_action, self.env_continue_action_dim, self.state_dim)
            self.optimizer_vae = SGD(self.VAE.parameters(), lr=3e-4)
        elif self.use_VAE2:
            ...
            self.VAE = Transformer(self.discrete_action_dim, self.continue_action, self.env_continue_action_dim, self.state_dim, 256, 32, 1, 8, dropout=0)
            self.optimizer_vae = SGD(self.VAE.parameters(), lr=3e-4)
        elif self.use_VAE3:
        ...
        #  ====================critic====================
        self.noise_dim = 50
        self.noise = np.random.randn(1, self.n_agents, self.noise_dim)
        if self.use_critic_RNN:
            ...
            self.critics = [CriticNetwork_overall_noise_T(critic_state_dim, self.noise_dim)] * 1
        elif self.use_S_ciritic:
            self.critics = [CriticNetwork_single_T(critic_state_dim)] * 1
        #  ====================critic====================
    ...
    def interact(self, MAX_EPISODES, EPISODES_BEFORE_TRAIN, NUMBER_OF_EVAL_EPISODES):
        ...
        while self.n_episodes < MAX_EPISODES:
            ...
            if self.replay_buffer.episode_num == self.batch_size:  # 在交换环境一定次数后开始训练
                self.n_episodes += 1
                # if self.n_episodes % 200 == 0 and self.count == 0:
                #     self.vae_train2()
                #     self.memory.buffer.clear()
                #     self.count = 1
    ...
    def vae_train2(self):
        ...
        for _ in range(eplosde):
            ...
            for index in BatchSampler(SequentialSampler(range(len(buffer_data))), 64, False):
                ...
                vae_mu1, vae_log_std1, vae_mu2, vae_log_std2, decode_action = self.VAE.forward(discrete_action, continue_action, state)
                recons_loss = nn.functional.mse_loss(continue_action, decode_action)
                kld_loss1 = torch.mean(0.5 * torch.sum(vae_mu1 ** 2 + vae_log_std1.exp() - vae_log_std1 - 1, dim=-1))
                kld_loss2 = torch.mean(0.5 * torch.sum(vae_mu2 ** 2 + vae_log_std2.exp() - vae_log_std2 - 1, dim=-1))
                # predict_loss = nn.functional.mse_loss(prediction_residual, true_residual)
                loss = recons_loss + kld_loss1 + kld_loss2
                if begin == 0:
                    print(loss)
    ...
    def train(self):
        ...
        if self.use_critic_RNN:
            ...
            noise = noise.repeat(self.batch_size, 1, 1)
        ...
        # advantage_mean = advantage.mean(axis=0, keepdim=True)
        # advantage_mean = advantage_mean.repeat(self.batch_size, 1, 1)
        # advantage_std = advantage.std(axis=0, keepdim=True)
        # advantage_std = advantage_std.repeat(self.batch_size, 1, 1)
        ...
        elif self.use_VAE3 or self.use_VAE:
            old_log_prob1 = old_log_prob[:, :, 0].reshape(self.batch_size, self.n_agents, -1)
            old_log_prob2 = old_log_prob[:, :, 1:self.continue_action+1].sum(dim=-1, keepdims=True)
            old_log_prob3 = old_log_prob[:, :, self.continue_action+1:].sum(dim=-1, keepdims=True)
            # old_log_prob = torch.cat([old_log_prob2, old_log_prob3], -1)
            #
            # discreate_action = torch.as_tensor(batch['loc_current_act_n'][:, :, 0], dtype=torch.int64).reshape(self.batch_size, self.n_agents, -1)
            # comb_old_log_prob2 = torch.gather(old_log_prob, -1, discreate_action)
        ...
        for _ in range(self.actors_epochs):
            ...
            if self.use_VAE2:
                ...
                new_log_prob3 = new_log_prob3.sum(dim=-1, keepdims=True)

                # new_log_prob = torch.cat([new_log_prob2, new_log_prob3], -1)
                # discreate_action = torch.as_tensor(batch['loc_current_act_n'][:, :, 0], dtype=torch.int64).reshape(self.batch_size, self.n_agents, -1)
                # comb_new_log_prob2 = torch.gather(new_log_prob, -1, discreate_action)

                ratio1 = (new_log_prob1 - old_log_prob1).exp()
                # ratio2 = (comb_new_log_prob2 - comb_old_log_prob2).exp()
                ratio2 = (new_log_prob2 - old_log_prob2).exp()
    ...
    def save_models(self, path):
        torch.save(self.VAE.state_dict(), path + str(self.InfdexofResult) + 'VAE.pth')
    ...
    def choose_action(self, original_state, evaluation):
        ...
        #     else:
        #         decode_continue = self.VAE.decoder(discrete_action, continue_action, state_var)
        ...
        if self.use_VAE2:
            ...
            if self.preheating and self.use_pca:
                ...
                decode_continue = self.VAE.decoder(discrete_action, current_continue, state_var)

            x_k = torch.clamp(decode_continue, -1, 1).reshape(self.n_agents, -1)
            action_var1 = action_var1.reshape(-1, 1)
            action_var = torch.cat([action_var1, x_k], -1)

            if self.use_cuda:
                actor_action = action_var.data.cpu().numpy()
                log_actor_action_discrete = policy_dist1.logits.data.cpu().numpy()
                log_actor_action = log_action_tanh.squeeze().data.cpu().numpy()
            else:
                actor_action = action_var.data.numpy()
                log_actor_action_discrete = policy_dist1.logits.data.numpy()
                log_actor_action = log_action_tanh.squeeze().data.numpy()
            ...
            log_action_tanh = torch.cat([log_action_tanh1, log_action_tanh2], -1).squeeze()
            discrete_action = action_var1

            # if not evaluation:
            #     epsilon = self.epsilon_end + (self.epsilon_start - self.epsilon_end) * \
            #               np.exp(-1. * self.n_episodes / self.epsilon_decay)
            #     noise = 0.1 * np.random.randn(1, self.n_agents, self.continue_action_dim) * epsilon
            #     current_continue += noise
            #     current_continue = to_tensor_var(current_continue, self.use_cuda)

            if self.preheating and self.use_pca:
                decode_continue = self.pca.fit_transform(current_continue)
                decode_continue = to_tensor_var(decode_continue, self.use_cuda)
            else:
                embedding_dis = self.VAE.embedding(discrete_action)
                decode_continue = self.VAE.decoder(embedding_dis, current_continue, state_var)
        ...
        log_actor_action1 = log_actor_action_discrete[0, arg, index]
        log_actor_action1 = log_actor_action1.reshape(-1, 1)

        discrete_action = torch.LongTensor(critic_action).reshape(1, -1)
        if self.use_cuda:
            discrete_action = discrete_action.cuda()
            embedding_dis = self.VAE.embedding(discrete_action)
            decode_continue = self.VAE.decoder(embedding_dis, current_continue, state_var)
        else:
            embedding_dis = self.VAE.embedding(discrete_action)
        ...
        if self.use_cuda:
            actor_action = action_var.data.cpu().numpy()
            current_continue = current_continue.cpu().numpy()
        else:
            actor_action = action_var.data.numpy()
        hybrid_action = deepcopy(actor_action)
        # ============find_log_action============
    ...
    def evaluate(self, EVAL_EPISODES):
        ...
        for i in range(EVAL_EPISODES):
            ...
            if i == EVAL_EPISODES-1 and done:
                ...
                savetxt('./CSV/results/MAPPO_VAE_Trans_10_'+str(self.InfdexofResult)+'.csv', arrayresults)
                savetxt('./CSV/loss/MAPPO_VAE_Trans_10_'+str(self.InfdexofResult)+'.csv', arrayloss)
                savetxt('./CSV/Server_constraints/MAPPO_VAE_Trans_10_'+str(self.InfdexofResult)+'.csv', arrayserver)
                savetxt('./CSV/Energy_constraints/MAPPO_VAE_Trans_10_'+str(self.InfdexofResult)+'.csv', arrayenergy)
                savetxt('./CSV/Time_constraints/MAPPO_VAE_Trans_10_' + str(self.InfdexofResult)+'.csv', arraytime)

                savetxt('./CSV/result_EC/MAPPO_VAE_Trans_10_'+str(self.InfdexofResult)+'.csv', result_EC)
                savetxt('./CSV/result_EX/MAPPO_VAE_Trans_10_'+str(self.InfdexofResult)+'.csv', result_EX)
                savetxt('./CSV/result_LT/MAPPO_VAE_Trans_10_'+str(self.InfdexofResult)+'.csv', result_LT)
                savetxt('./CSV/result_LX/MAPPO_VAE_Trans_10_' + str(self.InfdexofResult)+'.csv', result_LX)
    ...
    def evaluateAtTraining(self, EVAL_EPISODES):
        ...
        savetxt('./CSV/AtTraining/MAPPO_VAE_Trans_10_' + self.InfdexofResult+'.csv', arrayresults)
