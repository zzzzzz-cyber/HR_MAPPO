...
class MA_HPPO(object):
    ...
    def Data_collection(self):
        ...
        if self.replay_buffer.episode_num == self.batch_size:
            ...
            self.replay_buffer.buffer['loc_next_act_n'][:-1] = self.replay_buffer.buffer['loc_next_act_n'][1:]
            self.replay_buffer.buffer['loc_next_act_n'][-1] = next_actor_action

    def train(self):
        epsilon = self.epsilon_end + (self.epsilon_start - self.epsilon_end) * \
                  np.exp(-1. * self.n_episodes / self.epsilon_decay)
        noise = (2 * np.random.randn(self.batch_size, self.n_agents, self.noise_dim) - 1) * epsilon
        ...
        if self.use_Actor_RNN or self.use_Actor_RNN_soft1 or self.use_Actor_RNN_soft2:
            loc_current_act_n = batch['loc_current_act_n'].reshape(self.rnn_batch_size, MAX_STEPS, self.n_agents, -1)
            old_log_prob = old_log_prob.reshape(self.rnn_batch_size, MAX_STEPS, self.n_agents, -1)
            old_log_prob1 = old_log_prob[:, :, :, 0].reshape(self.rnn_batch_size, MAX_STEPS, self.n_agents, -1)
            old_log_prob2 = old_log_prob[:, :, :, 1:]
            discreate_action = torch.as_tensor(loc_current_act_n[:, :, :, 0], dtype=torch.int64).reshape(self.rnn_batch_size, MAX_STEPS, self.n_agents, -1)
            old_log_prob2 = torch.gather(old_log_prob2, -1, discreate_action)
            comb_old_log_prob = old_log_prob1 + old_log_prob2
            comb_old_log_prob = comb_old_log_prob.reshape(self.batch_size, self.n_agents, -1)
        elif self.use_3_tanh:
            ...
            old_log_prob1 = old_log_prob[:, :, 0].reshape(self.batch_size, self.n_agents, -1)
            old_log_prob2 = old_log_prob[:, :, 1].reshape(self.batch_size, self.n_agents, -1)
            old_log_prob3 = old_log_prob[:, :, 2].reshape(self.batch_size, self.n_agents, -1)
        for _ in range(self.actors_epochs):
            if self.use_soft_1_tanh:
                ...
                for t2 in range(MAX_STEPS):
                    new_action1, new_action2_mu, new_action2_std = self.new_actors[0](loc_current_obs_n[:, t2].reshape(self.rnn_batch_size * self.n_agents, -1))
                    new_action1 = new_action1.reshape(self.rnn_batch_size, self.n_agents, -1)
                    new_policy_dist1 = torch.distributions.Categorical(new_action1)
                    new_log_prob1 = new_policy_dist1.log_prob(loc_current_act_n[:, t2, :, 0]).reshape(self.rnn_batch_size, self.n_agents, -1)

                    new_action2_mu = new_action2_mu.reshape(self.rnn_batch_size, self.n_agents, -1)
                    new_action2_std = new_action2_std.reshape(self.rnn_batch_size, self.n_agents, -1)
                    new_policy_dist = torch.distributions.Normal(new_action2_mu, new_action2_std)
                    new_log_prob2 = new_policy_dist.log_prob(loc_current_act_n[:, t2, :, 1:])

                    discreate_action = torch.as_tensor(loc_current_act_n[:, t2, :, 0], dtype=torch.int64).reshape(self.rnn_batch_size, self.n_agents, -1)
                ...
                comb_new_log_prob = comb_new_log_prob.reshape(self.batch_size, self.n_agents, -1)
                ...
                comb_new_log_prob = []
                for t2 in range(MAX_STEPS):
                    new_action1, new_action2_mu, new_action2_std, new_action3_mu, new_action3_std = self.new_actors[0](loc_current_obs_n[:, t2].reshape(self.rnn_batch_size * self.n_agents, -1))
                    ...
                    new_action2_std = new_action2_std.reshape(self.rnn_batch_size, self.n_agents, -1)
                    new_policy_dist2 = torch.distributions.Normal(new_action2_mu, new_action2_std)
                    new_log_prob2 = new_policy_dist2.log_prob(loc_current_act_n[:, t2, :, 1].reshape(self.rnn_batch_size, self.n_agents, -1))

                    new_action3_mu = new_action3_mu.reshape(self.rnn_batch_size, self.n_agents, -1)
                    new_action3_std = new_action3_std.reshape(self.rnn_batch_size, self.n_agents, -1)
                    new_policy_dist3 = torch.distributions.Normal(new_action3_mu, new_action3_std)
                    new_log_prob3 = new_policy_dist3.log_prob(loc_current_act_n[:, t2, :, 2].reshape(self.rnn_batch_size, self.n_agents, -1))
                ...
                new_policy_dist3 = torch.distributions.Normal(new_action3_mu, new_action3_std)
                new_log_prob3 = new_policy_dist3.log_prob(batch['loc_current_act_n'][:, :, 2].reshape(self.batch_size, self.n_agents, -1))
                ...
                ratio1 = (new_log_prob1 - old_log_prob1).exp()
                ratio2 = (new_log_prob2 - old_log_prob2).exp()
                ratio3 = (new_log_prob3 - old_log_prob2).exp()
                # 近端策略优化裁剪目标函数公式的左侧项
                surr11 = ratio1 * advantage_actor
                surr12 = torch.clamp(ratio1, 1-self.eps, 1+self.eps) * advantage_actor
                ...
                comb_new_log_prob = []
                for t2 in range(MAX_STEPS):
                    new_action_mu, new_action_std = self.new_actors[0](loc_current_obs_n[:, t2].reshape(self.rnn_batch_size * self.n_agents, -1))
                    new_action_mu = new_action_mu.reshape(self.rnn_batch_size, self.n_agents, -1)
                    new_action_std = new_action_std.reshape(self.rnn_batch_size, self.n_agents, -1)

                    new_policy_dist = torch.distributions.Normal(new_action_mu, new_action_std)
                    new_log_prob = new_policy_dist.log_prob(loc_current_act_n[:, t2])
    ...
    def evaluate(self, EVAL_EPISODES):
        ...
        for i in range(EVAL_EPISODES):
            ...
            if i == EVAL_EPISODES-1 and done:
                ...
                savetxt('./CSV/results/MA_HPPO_'+str(self.InfdexofResult)+'.csv', arrayresults)
                savetxt('./CSV/loss/MA_HPPO_'+str(self.InfdexofResult)+'.csv', arrayloss)
                savetxt('./CSV/Server_constraints/MA_HPPO_'+str(self.InfdexofResult)+'.csv', arrayserver)
                savetxt('./CSV/Energy_constraints/MA_HPPO_'+str(self.InfdexofResult)+'.csv', arrayenergy)
                savetxt('./CSV/Time_constraints/MA_HPPO_' + str(self.InfdexofResult)+'.csv', arraytime)
    ...
    def evaluateAtTraining(self, EVAL_EPISODES):
        ...
        savetxt('./CSV/AtTraining/MA_HPPO_' + self.InfdexofResult+'.csv', arrayresults)
