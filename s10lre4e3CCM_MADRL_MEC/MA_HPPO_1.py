...
class MA_HPPO(object):
    ...
    def vae_train(self):
        vae_batch = self.replay_buffer.get_training_data(self.use_cuda)
        # for k in range(100):
        state = vae_batch['loc_current_obs_n'][self.replay_buffer.episode_num].reshape(MAX_STEPS * self.n_agents, -1)
        next_state = vae_batch['loc_obs_next_n'][self.replay_buffer.episode_num].reshape(MAX_STEPS * self.n_agents, -1)
        action = vae_batch['loc_current_act_n'][self.replay_buffer.episode_num].reshape(MAX_STEPS * self.n_agents, -1)
        ...
        kld_loss = torch.mean(0.5 * torch.sum(vae_mu ** 2 + vae_log_std.exp() - vae_log_std - 1, dim=1), dim=0)
    ...
    def train(self):
        ...
        if self.use_3_tanh or self.use_Actor_RNN:
            ...
            # old_log_prob2 = old_log_prob2.reshape(self.episodes_before_train, MAX_STEPS, self.n_agents, -1)
        ...
        for _ in range(self.epochs):
            ...
            if self.use_soft_1_tanh:
                ...
                ratio1 = (new_log_prob1 - old_log_prob1).exp()
                ratio2 = (new_log_prob2 - old_log_prob2).exp()
                ratio3 = (new_log_prob2 - old_log_prob2).exp()
                # 近端策略优化裁剪目标函数公式的左侧项
                surr11 = ratio1 * advantage_actor
                surr12 = torch.clamp(ratio1, 1-self.eps, 1+self.eps) * advantage_actor
