...
class TD3_VAE(object):
    def __init__(self, InfdexofResult, env, env_eval, n_agents, state_dim, action_dim, action_lower_bound, action_higher_bound,
        ...
        #  ====================critic====================
        if self.use_VAE:
            self.critics1 = [CriticNetwork_vae(critic_state_dim, self.vae_critic_action_dim)] * 1
            self.critics2 = [CriticNetwork_vae(critic_state_dim, self.vae_critic_action_dim)] * 1
        else:
    ...
    def train(self):
        ...
        next_action_var = torch.clamp(next_action_var + action_noise, -1, 1)
        ...
        target_value = self.reward_scale * rewards_var[:, 0, :] + self.reward_gamma * next_value
    ...
    def evaluate(self, EVAL_EPISODES):
        ...
        for i in range(EVAL_EPISODES):
            ...
            if i == EVAL_EPISODES-1 and done:
                ...
                savetxt('./CSV/results/TD3_VAE'+str(self.InfdexofResult)+'.csv', arrayresults)
                savetxt('./CSV/loss/TD3_VAE'+str(self.InfdexofResult)+'.csv', arrayloss)
                savetxt('./CSV/Server_constraints/TD3_VAE'+str(self.InfdexofResult)+'.csv', arrayserver)
                savetxt('./CSV/Energy_constraints/TD3_VAE'+str(self.InfdexofResult)+'.csv', arrayenergy)
                savetxt('./CSV/Time_constraints/TD3_VAE' + str(self.InfdexofResult)+'.csv', arraytime)
    ...
    def evaluateAtTraining(self, EVAL_EPISODES):
        ...
        savetxt('./CSV/AtTraining/TD3_VAE' + self.InfdexofResult+'.csv', arrayresults)
