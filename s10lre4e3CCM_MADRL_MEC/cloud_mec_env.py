...
class MecEnv(object):
    def __init__(self, n_agents, env_seed=None):
        ...
        self.min_max = [[MIN_POWER, MAX_POWER],
                        [MIN_GAIN, MAX_GAIN],
                        [MIN_ENE, MAX_ENE],
                        [MIN_SIZE, MAX_SIZE],
                        [MIN_CYCLE, MAX_CYCLE],
    ...
    def step_mec(self, action):
        ...
        # loss_reward = [Energy_n, Time_n, Energy_penalty, Time_penalty]
    ...
    def preprocessing(self, state):
        for i in range(state.shape[1]):
            state[:, i] = (state[:, i] - self.min_max[i][0]) / (self.min_max[i][1] - self.min_max[i][0])
        return state
