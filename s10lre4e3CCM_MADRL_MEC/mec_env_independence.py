...
if not record:
    ...
    loss_min_max = np.array(loss_min_max)
...
class MecEnv(object):
    ...
    def step_mec(self, action):
        ...
        if ENV_MODE == "H2":  # The hybrid actor critic mode as in the paper in reference number
            ...
            for i in range(self.n_agents):
                ...
                if x_n[SortedOff[i]] == 1 and counting < N_UNITS:
                    ...
                    MECtime[np.argmin(MECtime)] += Time_proc[SortedOff[i]]
        ...
        self.S_energy = np.clip(self.S_energy - Energy_n*1e-6 + np.random.normal(HARVEST_RATE, 0, size=self.n_agents)*1e-6, 0, MAX_ENE)  # 电池电量
        ...
        Reward = - 1 * (LAMBDA_E * normalize_reward_data[0] + LAMBDA_T * normalize_reward_data[2]) \
                 - 1 * (LAMBDA_E * normalize_reward_data[1] + LAMBDA_T * normalize_reward_data[3])

        # reward_data = self.normalize_loss(reward_data)
        # loss = - 1 * (LAMBDA_E * reward_data[0] + LAMBDA_T * reward_data[2])\
        #        - 1 * (LAMBDA_E * reward_data[1] + LAMBDA_T * reward_data[3])
        # loss = np.ones_like(loss) * np.sum(loss) / self.n_agents
