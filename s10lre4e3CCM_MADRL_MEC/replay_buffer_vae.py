...
class ReplayBuffer:
    def __init__(self, args, discrete_action_dim, continue_action_dim):
    ...
    def reset_buffer(self):
        self.buffer = {'loc_current_obs_n': np.empty([self.batch_size, self.n_agents, self.loc_state_dim]),
                       ...
                       'loc_current_discrete_n': np.empty([self.batch_size, self.n_agents, 1]),
                       'loc_current_continue_n': np.empty([self.batch_size, self.n_agents, self.continue_action_dim]),
                       'loc_current_log_act_n': np.empty([self.batch_size, self.n_agents, 1 + self.continue_action_dim]),
    ...
    def store_transition(self, loc_current_obs_n, glo_current_obs_1,
                         ...
                         loc_current_act_n, loc_current_discrete_n, loc_current_continue_n, loc_current_log_act_n, glo_current_act_1,
                         loc_obs_next_n, glo_obs_next_1,
                         glo_done_1):
        ...
        self.buffer['loc_current_discrete_n'][self.episode_num] = loc_current_discrete_n
        self.buffer['loc_current_continue_n'][self.episode_num] = loc_current_continue_n
